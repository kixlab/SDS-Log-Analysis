...
def chi_square_feature(cid_pattern_list, cid_user_cnt, interested_cids = None, printEval = False):
	...
	for cid in cid_pattern_list:
		...
		resultMap[cid] = sorted(list(scores.items()), key = lambda x: x[1], reverse = True)
		if printEval:
...
def mutual_info_feature(cid_pattern_cnt, cid_user_cnt = None, interested_cids = None):
	if not cid_user_cnt:
		...
		for cid in cid_pattern_cnt:
			...
			cid_user_cnt[cid] = sum([x[1] for x in list(cid_pattern_cnt[cid].items())])
...
if __name__ == '__main__':
	print((mutual_info_feature(pickle.load(open('cid_pattern_cnt.pkl')), pickle.load(open('cid_user_cnt.pkl')), \
