import multiprocessing, json, sys, math, subprocess, os, glob, pickle, time
from array import *
import datetime
...
class myThread (multiprocessing.Process):
	...
	def __init__(self, threadID, prefix, sid_seq, sfrom, sto, idfMap):
		...
		# self.fo = open(prefix+'_'+str(sfrom[0]), 'wb')
	...
	def run(self):
		...
		self.fo = open(self.prefix+'_'+str(self.sfrom[0]), 'w')
...
def calDist(inputPath, sidsPath, outputPath, tmpPrefix = '', idfMapPath = None):
	...
	for line in open(inputPath):
		...
		line = line[:-1].split(')')
		if idfMap:
			curSeq = [(item[0], int(idfMap[item[0]] * int(item[1]))) for item in [x.split('(') for x in line]]
		else:
			curSeq = [(item[0], int(item[1])) for item in [x.split('(') for x in line]]
...
def partialMatrix(sids, idfMap, ngramPath, tmpPrefix, outputPath, realSid = False):
	...
	for server in servers:
		if (start >= total):
			break
		pickle.dump([sids[start:start+step], sids], open('%s%ssid_%s.pkl' % (outputPath, tmpPrefix, server), 'wb'))
...
def fullMatrix(inputPath, outputPath):
	...
	for server in servers:
		if (start >= total):
			break
		pickle.dump([sids[start:start+step], sids], open('%ssid_%s.pkl' % (outputPath, server), 'wb'))
