...
def getSidNgramMap(inputPath, sids = None):
	...
	for line in open(inputPath):
		...
		line = line[:-1].split(')')
		sid_seq[sid] = dict([(item[0], int(item[1])) for item in [x.split('(') for x in line]])
	return sid_seq
...
def splitCluster(xxx_todo_changeme, matrix):
	(baseCluster, diameter, baseSum, cid) = xxx_todo_changeme
	newCluster = []
